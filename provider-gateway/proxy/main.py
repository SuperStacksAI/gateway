...
app = FastAPI(title="Provider Gateway")

DATABASE_URL = os.environ["DATABASE_URL"]
REDIS_URL = os.environ.get("REDIS_URL", "redis://redis:6379")

PROVIDER_BASE_URL = os.environ["PROVIDER_BASE_URL"].rstrip("/")
...
async def proxy_stream(
    vllm_url: str, headers: dict, body: dict,
    user_id: str, api_key_id: str | None,
        ...
        async with client.stream("POST", vllm_url, json=body, headers=headers) as resp:
            ...
            try:
                ...
                if prompt_tokens or completion_tokens:
                    ...
                    asyncio.create_task(
                        log_usage(user_id, api_key_id, body.get("model", ""), prompt_tokens, completion_tokens, cost)
                    )


def build_provider_headers() -> dict:
    return {
...
async def chat_completions(request: Request):
    ...
    provider_url = f"{PROVIDER_BASE_URL}/chat/completions"
    provider_headers = build_provider_headers()

    if body.get("stream", False):
        return StreamingResponse(
            proxy_stream(provider_url, provider_headers, body, key_data["user_id"], key_data["id"]),
            media_type="text/event-stream",
        )

    async with httpx.AsyncClient(timeout=120) as client:
        resp = await client.post(provider_url, json=body, headers=provider_headers)
        if resp.status_code != 200:
        ...
        cost = (prompt_tokens / 1_000_000 * INPUT_PRICE_PER_M +
                completion_tokens / 1_000_000 * OUTPUT_PRICE_PER_M)
        asyncio.create_task(
            log_usage(key_data["user_id"], key_data["id"], body.get("model", ""), prompt_tokens, completion_tokens, cost)
        )
